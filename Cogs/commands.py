...
Original file is located at
    https://colab.research.google.com/drive/12l8sioSFxSzM_b69r36plqZlFcLTMCCD
"""
...
class hello(commands.Cog):
        ...
        if 시즌 == PREDICT_SEASON:
            ...
            if today.weekday() != 6:
                ...
                async def see1button_callback(interaction:discord.Interaction): # 순위표 버튼을 눌렀을 때의 반응!
                    refp = db.reference(f'{PREDICT_SEASON}/예측포인트/{interaction.user.name}')
                    originr = refp.get()
                    point = originr["포인트"]
    ...
    async def 포인트(self, interaction: discord.Interaction):
        ref = db.reference(f'{PREDICT_SEASON}/예측포인트')
        points = ref.get()
    ...
    async def 정상화(self, interaction: discord.Interaction):
        if interaction.user.name == "toe_kyung":
            ref = db.reference(f'{PREDICT_SEASON}/예측포인트변동로그')
            ...
            if snapshot:
                ...
                if latest_log_data:
                    for name, log in latest_log_data.items():
                        ref2 = db.reference(f'{PREDICT_SEASON}/예측포인트/{name}')
            ...
            ref3 = db.reference(f'{PREDICT_SEASON}/예측포인트변동로그/{date}/{time}')
            ref3.delete()
            ...
            nickname not in [loser['name'] for loser in p.prediction_votes['lose']]):
                ...
                for winner in p.prediction_votes['win']:
                    if winner['name'] == nickname:
                        ...
                        ref2 = db.reference(f'{PREDICT_SEASON}/예측포인트/{winner["name"]}/베팅포인트')
                ...
                for loser in p.prediction_votes['lose']:
                    if loser['name'] == nickname:
                        ...
                        ref2 = db.reference(f'{PREDICT_SEASON}/예측포인트/{loser["name"]}/베팅포인트')
                        bettingPoint = ref2.get()
                        info = ref.get()
    ...
    async def 승리(self, interaction: discord.Interaction, 이름:str, 포인트:int, 배율:float, 베팅금액:int):
        userembed = discord.Embed(title="메세지", color=discord.Color.light_gray())
        if interaction.user.name == "toe_kyung":
            ref = db.reference(f'{PREDICT_SEASON}/예측포인트/{이름}')
    ...
    async def 확성기(self, interaction: discord.Interaction, 익명여부:str, 메세지:str):
        ...
        if 익명여부 == '익명':
            ...
            need_point = 100
        ref = db.reference(f'{PREDICT_SEASON}/예측포인트/{interaction.user.name}')
        originr = ref.get()
        point = originr["포인트"]
        bettingPoint = originr["베팅포인트"]
        if point - bettingPoint < need_point:
