    ...
    async def check_guess(self, player, guess):
        ...
        if strikes == 3:
            ...
            if baseball_winner.name == self.challenger:
                result = True
            elif baseball_winner.name == self.opponent:
                result = False
...
class hello(commands.Cog):
    ...
    async def commands(self, interaction: discord.Interaction):
        exclude = {}
        commands_list = [cmd for cmd in self.bot.tree.get_commands() if cmd.name not in exclude and isinstance(cmd, app_commands.Command)]
        commands_list.sort(key=lambda x: x.name)
        commands_embed = discord.Embed(title="명령어 목록", color=discord.Color.green())
        for cmd in commands_list:
            commands_embed.add_field(name=f"</{cmd.mention}>", value=cmd.description, inline=False)
