...
class hello(commands.Cog):
    ...
    @app_commands.command(name="지모점수",description="현재 지모의 점수를 보여줍니다")
    async def 지모점수(self,interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 지모점수 요청 수행")
        puuid = await get_summoner_puuid(JIMO_NAME,JIMO_TAG)
        id = await get_summoner_id(puuid)
        rank = await get_summoner_ranks(id)
        wins = rank["wins"]
        losses = rank["losses"]
        winrate = round((wins / (wins + losses))* 100,2)
        await interaction.response.send_message(f'{rank["tier"]} {rank["rank"]} {rank["leaguePoints"]}LP\n'
    ...
    '''
    @app_commands.command(name="공식다이아",description="아프로스의 공식 다이아 수를 보여줍니다")
    async def 공식다이아(self, interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 공식다이아 요청 수행")
        puuid = await get_summoner_puuid(JIMO_NAME,JIMO_TAG)
        id = await get_summoner_id(puuid)
        rank = await get_summoner_ranks(id)
        count = 0
        if rank and rank["tier"] == "DIAMOND":
            count += 1
        await interaction.response.send_message(f'아프로스 내의 공식 다이아 수는 {count}명 입니다.')

    @app_commands.command(name="순수실력",description="순수실력을 보여줍니다")
    @app_commands.describe(닉네임='소환사 닉네임',태그='소환사 태그 ex)KR1')
    async def 순수실력(self, interaction: discord.Interaction, 닉네임:str, 태그:str):
        print(f"{interaction.user}가 요청한 순수실력 요청 수행")
        RNAME = 닉네임
        TLINE = 태그
        RNAME = RNAME.strip()
        TLINE = TLINE.strip()
        ...
        if RNAME.lower() == JIMO_NAME.lower():
            id = await get_summoner_id(puuid)
            rank = await get_summoner_ranks(id)
            if rank['tier'] == 'DIAMOND':
                ...
                sunsu_rank = 'DIAMOND'
            await interaction.response.send_message(f"{RNAME}의 순수실력 :{sunsu_rank}")
        else:
            id = await get_summoner_id(puuid)
            rank = await get_summoner_ranks(id)
            if rank == []:
                await interaction.response.send_message(f'{RNAME}의 순수실력 측정 불가 (솔로랭크 전적 없음)')
                return
            tier_num = tier_to_number(rank['tier'],rank['rank'],rank['leaguePoints'])
            if rank['tier'] == "CHALLENGER" or rank['tier'] == "MASTER" or rank['tier'] == "GRANDMASTER":
                await interaction.response.send_message(f"{RNAME}의 순수실력 :{rank['tier']}")
            elif RNAME.lower() == "hide on bush" and TLINE == "KR1":
                await interaction.response.send_message(f"{RNAME}의 순수실력 :CHALLENGER")
            else:
                sunsu_rank = get_tier_name((tier_num - 300) // 400)
                await interaction.response.send_message(f"{RNAME}의 순수실력 :{sunsu_rank}")
    '''
    ...
    @app_commands.command(name="전판이김",description="지모가 전판을 이겼을까요?")
    async def 전판이김(self, interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 전판이김? 요청 수행")
        puuid = await get_summoner_puuid(JIMO_NAME,JIMO_TAG)
        matchid = await get_summoner_recentmatch_id(puuid)
        matchinfo = await get_summoner_matchinfo(matchid)
        result = '패배'
        if(matchinfo['info']['teams'][0]['win'] == False):
            for i in range(5,10):
                if(matchinfo['info']['participants'][i]["summonerName"].replace(" ","").lower()==JIMO_NAME):
                    damage = matchinfo['info']['participants'][i]['totalDamageDealtToChampions']
                    kills = matchinfo['info']['participants'][i]['kills']
                    triplekill = matchinfo['info']['participants'][i]['tripleKills']
                    quadrakill = matchinfo['info']['participants'][i]['quadraKills']
                    pentakill = matchinfo['info']['participants'][i]['pentaKills']
                    result = '승리'
        elif(matchinfo['info']['teams'][0]['win']== True):
            for i in range(0,5):
                if(matchinfo['info']['participants'][i]["summonerName"].replace(" ","").lower()==JIMO_NAME):
                    damage = matchinfo['info']['participants'][i]['totalDamageDealtToChampions']
                    kills = matchinfo['info']['participants'][i]['kills']
                    ...
                    result = '승리'
        if result == '패배':
            if interaction.user.name == 'songjinho6533':
                r = random.randrange(1,11)
                if(r < 4):
                    await interaction.response.send_message(f'니네 애미한테 물어봐')
                elif(r < 8):
                    await interaction.response.send_message(f'씨발 진호야')
                else:
                    await interaction.response.send_message(f'진호 애미 터짐?')
            else:
                r = random.randrange(1,11)
                if(r < 8):
                    await interaction.response.send_message(f'대가리 터짐?')
                elif(r < 9):
                    await interaction.response.send_message(f'눈 안뜸?')
                elif(r < 11) :
                    await interaction.response.send_message(f'에휴 또 놀리네')

        elif result == '승리':
            r = random.randrange(1,11)
            if(r < 3):
                await interaction.response.send_message(f'어 {damage}딜 넣고 이겼어')
            elif(r < 5):
                await interaction.response.send_message(f'아직 복구까지 멀었어')
            elif(r < 8):
                if pentakill != 0:
                    await interaction.response.send_message(f'어 펜타킬 하고 이겼어')
                elif quadrakill != 0:
                    await interaction.response.send_message(f'어 쿼드라킬 하고 이겼어')
                elif triplekill != 0:
                    await interaction.response.send_message(f'어 트리플킬 하고 이겼어')
                else:
                    await interaction.response.send_message(f'개버스탔는데????')
            elif(r < 11):
                await interaction.response.send_message(f'어 {kills}킬 따고 이겼어')

    '''
    @app_commands.command(name="화남정도",description="얼마나 화났을까요?")
    async def 화남정도(self, interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 화남정도 요청 수행")

        # 현재 날짜 및 시간 가져오기
        current_datetime = datetime.now()

        if current_datetime <= SEASON_CHANGE_DATE: #시즌 종료 전
            season = "시즌14-1"
        else:
            season = "시즌14-2"

        ref = db.reference(f'{season}/점수변동/지모')
        lp_difference = ref.get()
        matches = []
        ...
        for date, entries in lp_difference.items():
            for time, entry in entries.items():
                ...
                #print(time)
                #print(entry['LP 변화량'])
                matches.append((date+" "+time, entry['LP 변화량']))

        recent_matches = matches[:]

        # 승/패 여부 기록
        results = []
        for _, lp_change in recent_matches:
            if lp_change > 0:
                results.append('승')
            elif lp_change <= 0:
                results.append('패')
        ...
        angryStatus = 80
        # 결과 출력
        for idx, result in enumerate(reversed(results), start=1):
            if idx == 1:  # 첫 번째 경기인 경우
                first_result = result  # 첫 번째 승/패 여부를 기록
            elif idx == 2:
        ...
        last_match_time_str = recent_matches[-1][0]
        last_match_time = datetime.strptime(last_match_time_str, '%Y-%m-%d %H:%M:%S')  # 문자열을 datetime 객체로 변환
        time_difference = current_time - last_match_time

        # 경과한 시간을 시간 단위로 변환
        hours_elapsed = time_difference.days * 24 + time_difference.seconds // 3600


        winrate = round((wins / (wins + losses)) * 100,2)

        angryStatus -= winrate
        angryStatus -= hours_elapsed
        if first_result == '패':
            angryStatus += 30
        if second_result == '패':
            angryStatus += 20
        ...
        if angryStatus < 0: angryStatus = 0

        await interaction.response.send_message(f"화남정도는 {angryStatus}% 입니다.")
        '''
    '''
    @app_commands.command(name="화남정도",description="얼마나 화났을까요?")
    async def 화남정도(self, interaction: discord.Interaction):
        ...
        ref = db.reference('화남정도/화남정도')
        angryStatus = ref.get()

        await interaction.response.send_message(f"화남정도는 {angryStatus}% 입니다.")


    @app_commands.command(name="지모연승",description="지모의 연승 횟수를 보여줍니다")
    async def 지모연승(self, interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 지모연승 요청 수행")
        # 현재 날짜 및 시간 가져오기
        current_datetime = datetime.now()
        ...
        win_streak = 0
        # 결과 출력
        for _, result in enumerate(reversed(results), start=1):
            if result == '승':
                win_streak += 1
            else:
                break
        ...
        if win_streak != 0:
            await interaction.response.send_message(f"지모의 현재 연승 횟수: {win_streak}연승")
        else:
            await interaction.response.send_message("지모는 현재 연승중이 아닙니다.")

    @app_commands.command(name="지모연패",description="지모의 연패 횟수를 보여줍니다")
    async def 지모연패(self, interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 지모연패 요청 수행")
        # 현재 날짜 및 시간 가져오기
        current_datetime = datetime.now()
        ...
        if loss_streak != 0:
            await interaction.response.send_message(f"지모의 현재 연패 횟수: {loss_streak}연패")
        else:
            await interaction.response.send_message("지모는 현재 연패중이 아닙니다.")

    @app_commands.command(name="지모전적",description="지모의 솔랭 최근 30경기 전적을 보여줍니다")
    async def 지모전적(self, interaction: discord.Interaction):
        print(f"{interaction.user}가 요청한 지모전적 요청 수행")
        # 현재 날짜 및 시간 가져오기
        current_datetime = datetime.now()
        ...
        for date, entries in lp_difference.items():
            for entry in entries.values():
                lp_change = entry.get('LP 변화량')
                entries_list.append((date, lp_change))

        # 최근 30경기 선택
        recent_30_games = entries_list[:30]

        # 승/패 여부 기록
        results = []
        for date, lp_change in recent_30_games:
            if lp_change > 0:
                results.append('승')
            elif lp_change <= 0:
                results.append('패')

        wins = results.count('승')
        losses = results.count('패')

        winrate = round((wins / (wins + losses))* 100,2)

        await interaction.response.send_message(f"지모의 최근 30경기\n"
                        f"승: {wins}, 패: {losses}, 승률: {winrate}%")
    '''
    ...
    async def 베팅(self, interaction: discord.Interaction, 이름:str, 포인트:int):
        ...
        if 포인트 <= 0:
            ...
            return
        if 이름 == "지모":
            if p.jimo_winbutton.disabled == True:
                await interaction.response.send_message("지금은 지모에게 베팅할 수 없습니다!",ephemeral=True)
                return
            nickname = interaction.user.name
            if (nickname not in [winner['name'] for winner in p.prediction_votes['win']] and
            nickname not in [loser['name'] for loser in p.prediction_votes['lose']]):
                await interaction.response.send_message(f"승부예측 후 이용해주세요",ephemeral=True)
            else:
                for winner in p.prediction_votes['win']:
                        ...
                        if winner['points'] != 0:
                            if info['포인트'] - bettingPoint < 포인트:
                                ...
                                if anonymbool:
                                  ...
                                  await interaction.response.send_message(f"지모의 승리에 {포인트}포인트 베팅 완료!",ephemeral=True)
                                else:
                ...
                # 패배 예측에서 닉네임 찾기
                for loser in p.prediction_votes['lose']:
                    if loser['name'] == nickname:
                        ...
                        return
        if 이름 == "Melon":
            if p.melon_winbutton.disabled == True:
                await interaction.response.send_message("지금은 Melon에게 베팅할 수 없습니다!",ephemeral=True)
                return
            nickname = interaction.user.name
            if (nickname not in [winner['name'] for winner in p.prediction_votes2['win']] and
            nickname not in [loser['name'] for loser in p.prediction_votes2['lose']]):
                await interaction.response.send_message(f"승부예측 후 이용해주세요",ephemeral=True)
            else:
                for winner in p.prediction_votes2['win']:
                    if winner['name'] == nickname:
                        ref = db.reference(f'{current_predict_season}/예측포인트/{winner["name"]}')
                        ref2 = db.reference(f'{current_predict_season}/예측포인트/{winner["name"]}/베팅포인트')
                        bettingPoint = ref2.get()
                        ...
                        if winner['points'] != 0:
                            if info['포인트'] - bettingPoint < 포인트:
                                ...
                                userembed = discord.Embed(title="메세지", color=discord.Color.blue())
                                if anonymbool:
                                  await place_bet(self.bot,"Melon","승리",포인트)
                                  await interaction.response.send_message(f"Melon의 승리에 {포인트}포인트 베팅 완료!",ephemeral=True)
                                else:
                                  userembed.add_field(name="",value=f"{nickname}님이 Melon의 승리에 {포인트}포인트만큼 추가 베팅하셨습니다!", inline=True)
                                  await interaction.response.send_message(embed=userembed)
                        else:
                            if info['포인트'] - bettingPoint < 포인트:
                                await interaction.response.send_message(f"포인트가 부족합니다!\n현재 포인트: {info['포인트'] - bettingPoint}(베팅 금액 {bettingPoint}P) 제외",ephemeral=True)
                                ...
                                if anonymbool:
                                  ...
                                  userembed.add_field(name="",value=f"{nickname}님이 Melon의 승리에 {포인트}포인트만큼 베팅하셨습니다!", inline=True)
                                  await interaction.response.send_message(embed=userembed)
                        p.prediction_embed = discord.Embed(title="예측 현황", color=discord.Color.blue())
                        if anonymbool: # 익명 투표 시
                          win_predictions = "\n".join(
                        ...
                        if p.current_message_melon:
                            await p.current_message_melon.edit(embed = p.prediction_embed)
                        return

                # 패배 예측에서 닉네임 찾기
                for loser in p.prediction_votes2['lose']:
                    if loser['name'] == nickname:
                        ref = db.reference(f'{current_predict_season}/예측포인트/{loser["name"]}')
                        ref2 = db.reference(f'{current_predict_season}/예측포인트/{loser["name"]}/베팅포인트')
                        bettingPoint = ref2.get()
                        ...
                        if loser['points'] != 0:
                            if info['포인트'] - loser['points'] < 포인트:
                                ...
                                userembed = discord.Embed(title="메세지", color=discord.Color.blue())
                                if anonymbool:
                                  await place_bet(self.bot,"Melon","패배",포인트)
                                  await interaction.response.send_message(f"Melon의 패배에 {포인트}포인트 베팅 완료!",ephemeral=True)
                                else:
                                  userembed.add_field(name="",value=f"{nickname}님이 Melon의 패배에 {포인트}포인트만큼 추가 베팅하셨습니다!", inline=True)
                                  await interaction.response.send_message(embed=userembed)
                        else:
                            if info['포인트'] - bettingPoint < 포인트:
                                await interaction.response.send_message(f"포인트가 부족합니다!\n현재 포인트: {info['포인트'] - bettingPoint}(베팅 금액 {bettingPoint}P) 제외",ephemeral=True)
                                ...
                                if anonymbool:
                                  ...
                                  userembed.add_field(name="",value=f"{nickname}님이 Melon의 패배에 {포인트}포인트만큼 베팅하셨습니다!", inline=True)
                                  await interaction.response.send_message(embed=userembed)
                        p.prediction_embed = discord.Embed(title="예측 현황", color=discord.Color.blue())
                        if anonymbool: # 익명 투표 시
                          win_predictions = "\n".join(
